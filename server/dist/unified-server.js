...
let SHARK_EVOLUTIONS = [];
function getSharkScaleForType(type) {
    if (!type || !SHARK_EVOLUTIONS || SHARK_EVOLUTIONS.length === 0)
        return 1.0;
...
const ROOTS = [
    ...
    (0, path_1.join)(process.cwd(), 'server'),
];
...
function tryResolve(pathname) {
    ...
    for (const root of roots) {
        ...
        if ((0, fs_2.existsSync)(full) && (0, fs_2.statSync)(full).isFile())
            return full;
        if ((0, fs_2.existsSync)(full) && (0, fs_2.statSync)(full).isDirectory()) {
            const idx = (0, path_1.join)(full, 'index.html');
            if ((0, fs_2.existsSync)(idx))
                return idx;
        }
    }
    const fallback = (0, path_1.join)(ROOTS[0], 'index.html');
    return (0, fs_2.existsSync)(fallback) ? fallback : null;
}
...
async function getUsers() {
    try {
        const buf = await fs_1.promises.readFile(USERS_FILE, 'utf8');
...
async function saveUsers(users) {
    ...
    await fs_1.promises.writeFile(USERS_FILE, JSON.stringify(users, null, 2));
}
// ============================================
...
httpServer.on('request', async (req, res) => {
    ...
    (0, fs_2.createReadStream)(file).pipe(res);
});
...
function bubbleHitsShark(p, bx, by) {
    ...
    for (let oy = -3; oy <= 3; oy++) {
        for (let ox = -3; ox <= 3; ox++) {
            ...
            if (x < 0 || y < 0 || x >= SHARK_MASK_SIZE || y >= SHARK_MASK_SIZE)
                continue;
            if (playerMask[y * SHARK_MASK_SIZE + x] !== 0)
                return true;
        }
    }
    ...
    for (const offset of offsets) {
        ...
        if (sx < 0 || sy < 0 || sx >= SHARK_MASK_SIZE || sy >= SHARK_MASK_SIZE)
            continue;
        if (playerMask[sy * SHARK_MASK_SIZE + sx] !== 0)
            return true;
    }
    return false;
}
...
function sharkCollidesWithShark(shark1, shark2) {
    ...
    const kernelFor = (sf) => (sf >= 1.3 ? 2 : 1); // 3x3 or 5x5
...
io.on('connection', (socket) => {
    ...
    socket.on('client:ping', (t0) => { socket.emit('server:pong', t0); });
        ...
        if (me) {
            ...
            // unguarded: rely on per-player masks (per-player masks checked inside sharkCollidesWithShark)
            {
                ...
                if (blocked && hitOther) {
                    ...
                    if (meSock)
                        meSock.emit('shark:collision', { damage: SHARK_COLLISION_DAMAGE });
                    if (otherSock)
                        otherSock.emit('shark:collision', { damage: SHARK_COLLISION_DAMAGE });
