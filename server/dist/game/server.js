...
const maskGenerator_1 = require("../utils/maskGenerator");
// Map configuration
const MAP_SIZE = 4000;
...
const BUBBLE_SEEDS = Array.from({ length: 28 }, () => ({ left: Math.random() * 100, delay: -Math.random() * 14 }));
...
const lastCollisionDamageAt = new Map();
// Keep last known username per socket (used to respawn after death removal)
const usernames = new Map();
// Parsed alpha masks (1=opaque, 0=transparent)
let sharkMask = null; // length SHARK_SIZE*SHARK_SIZE
let foodMask = null; // length FOOD_SIZE*FOOD_SIZE
// Combat bookkeeping for assists and kill awards
const ASSIST_WINDOW_MS = 20000; // 20 seconds
...
let LEVEL_STEPS = [];
let SHARK_EVOLUTIONS = []; // Ordered list of shark filenames
const sharkMasks = new Map(); // Cache of loaded shark masks
const sharkMouthOffsets = new Map(); // Mouth positions per shark
const sharkTailOffsets = new Map(); // Tail positions per shark
...
async function loadLevelProgression() {
    try {
...
async function loadSharkEvolutions() {
    try {
...
function computeLevel(score) {
    if (!LEVEL_STEPS || LEVEL_STEPS.length === 0)
        return 1;
    let lvl = 1;
...
function getSharkTypeForLevel(level) {
    if (!SHARK_EVOLUTIONS || SHARK_EVOLUTIONS.length === 0)
        return 'Baby Shark.png';
...
function formatSharkName(sharkType) {
    return sharkType.replace('.png', '');
}
// Load a specific shark's mask from PNG and compute mouth/tail positions
async function loadSharkMask(sharkFilename) {
    if (sharkMasks.has(sharkFilename))
        return; // already loaded
    try {
        // Generate mask from PNG alpha channel
        const pngPath = path.join('server', 'sharks', sharkFilename);
        const mask = await (0, maskGenerator_1.generateMaskFromPNG)(pngPath, SHARK_MASK_SIZE);
        sharkMasks.set(sharkFilename, mask);
        // Compute mouth and tail positions
        try {
            const mouth = (0, maskGenerator_1.computeMouthAnchorFromMask)(mask, SHARK_MASK_SIZE);
        ...
        try {
            const tail = (0, maskGenerator_1.computeTailAnchorFromMask)(mask, SHARK_MASK_SIZE);
...
async function loadServerMasks() {
    try {
        // Generate baby shark mask from PNG
        const babySharkPng = path.join('server', 'sharks', 'Baby Shark.png');
        const rawShark = await (0, maskGenerator_1.generateMaskFromPNG)(babySharkPng, SHARK_MASK_SIZE);
        sharkMask = rawShark;
        try {
            const mouth = (0, maskGenerator_1.computeMouthAnchorFromMask)(sharkMask, SHARK_MASK_SIZE);
    ...
    try {
        ...
        const rawFood = await (0, maskGenerator_1.generateMaskFromPNG)(foodPng, 64);
...
function pixelPerfectOverlap(me, food) {
    const playerMask = getPlayerMask(me);
    if (!playerMask || !foodMask)
        return false;
    // Coarse prune with per-shark scale
    const sf = getSharkScaleForType(me.sharkType);
    const effR = SHARK_HALF * sf;
    // Shark center in world space: position is top-left, so add HALF of the RENDERED size
    const renderedHalf = SHARK_HALF * sf;
    const cx = me.x + renderedHalf;
    const cy = me.y + renderedHalf;
    ...
    for (let fy = 0; fy < FOOD_SIZE; fy++) {
        for (let fx = 0; fx < FOOD_SIZE; fx++) {
            ...
            if (!debugLogged && Math.abs(vx) < 50 && Math.abs(vy) < 50) {
                console.log(`🔍 Food collision check:
  Shark: pos=(${me.x.toFixed(1)}, ${me.y.toFixed(1)}) center=(${cx.toFixed(1)}, ${cy.toFixed(1)}) angle=${(a * 180 / Math.PI).toFixed(1)}° scale=${sf.toFixed(2)}
  Food: pos=(${food.x.toFixed(1)}, ${food.y.toFixed(1)})
  Food pixel: world=(${wx.toFixed(1)}, ${wy.toFixed(1)}) local=(${vx.toFixed(1)}, ${vy.toFixed(1)})
  After rotation: (${rx.toFixed(1)}, ${ry.toFixed(1)}) flip=${flipY}
  After flip: (${lx.toFixed(1)}, ${ly.toFixed(1)})
  Mask coords: (${mx.toFixed(1)}, ${my.toFixed(1)}) -> (${mxi}, ${myi})
  Scale factor: ${scale.toFixed(3)}`);
            ...
            if (mxi >= 0 && myi >= 0 && mxi < SHARK_MASK_SIZE && myi < SHARK_MASK_SIZE) {
                if (playerMask[myi * SHARK_MASK_SIZE + mxi] !== 0) {
                    console.log(`✅ COLLISION DETECTED at mask (${mxi}, ${myi})`);
                    return true;
                }
            }
            ...
            for (let dy = -2; dy <= 2; dy++) {
                for (let dx = -2; dx <= 2; dx++) {
                    ...
                    if (tx >= 0 && ty >= 0 && tx < SHARK_MASK_SIZE && ty < SHARK_MASK_SIZE) {
                        if (playerMask[ty * SHARK_MASK_SIZE + tx] !== 0)
                            return true;
                    }
                }
            }
...
async function checkAndHandleEvolution(player) {
...
function handleConsume(me, food) {
...
function spawnFoodDistributed(tries = 20) {
    ...
    const food = { id: nextFoodId++, x: best.x, y: best.y, vx: Math.cos(ang) * speed, vy: Math.sin(ang) * speed };
    gameState.foods[food.id] = food;
    return food;
}
function bubbleHitsShark(p, bx, by) {
    ...
    for (let sy = -2; sy <= 2; sy++) {
        for (let sx = -2; sx <= 2; sx++) {
            ...
            if (ux >= 0 && uy >= 0 && ux < SHARK_MASK_SIZE && uy < SHARK_MASK_SIZE) {
                if (playerMask[uy * SHARK_MASK_SIZE + ux] !== 0)
                    return true;
            }
        }
    }
...
function sharkCollidesWithShark(shark1, shark2) {
    ...
    const stepFor = (sf) => Math.max(1, Math.floor(1 / (SHARK_SCALE * sf)));
    ...
    const kernelFor = (sf) => (sf >= 1.28 ? 3 : sf >= 1.15 ? 2 : 1); // 3=>7x7, 2=>5x5, 1=>3x3
    ...
    const samplesHit = (srcMask, srcCenterX, srcCenterY, srcCos, srcSin, srcFlipY, srcScale, dstMask, dstCenterX, dstCenterY, dstCosInv, dstSinInv, dstFlipY, dstScale, step, kernel) => {
        for (let my = 0; my < SHARK_MASK_SIZE; my += step) {
            for (let mx = 0; mx < SHARK_MASK_SIZE; mx += step) {
                if (srcMask[my * SHARK_MASK_SIZE + mx] === 0)
                    continue; // only colored pixels
                ...
                if (xi >= 0 && yi >= 0 && xi < SHARK_MASK_SIZE && yi < SHARK_MASK_SIZE) {
                    if (dstMask[yi * SHARK_MASK_SIZE + xi] !== 0)
                        return true;
                    ...
                    for (let oy = -kernel; oy <= kernel; oy++) {
                        for (let ox = -kernel; ox <= kernel; ox++) {
                            ...
                            if (tx < 0 || ty < 0 || tx >= SHARK_MASK_SIZE || ty >= SHARK_MASK_SIZE)
                                continue;
                            if (dstMask[ty * SHARK_MASK_SIZE + tx] !== 0)
                                return true;
                        }
                    }
                    ...
                    for (const off of fracOffsets) {
                        ...
                        if (sx < 0 || sy < 0 || sx >= SHARK_MASK_SIZE || sy >= SHARK_MASK_SIZE)
                            continue;
                        if (dstMask[sy * SHARK_MASK_SIZE + sx] !== 0)
                            return true;
                    }
                }
            }
    ...
    if (samplesHit(mask2, cx2, cy2, cos2, sin2, flipY2, scale2, mask1, cx1, cy1, cos1Inv, sin1Inv, flipY1, scale1, step2, k1))
        return true;
    ...
    if (samplesHit(mask1, cx1, cy1, cos1, sin1, flipY1, scale1, mask2, cx2, cy2, cos2Inv, sin2Inv, flipY2, scale2, step1, k2))
        return true;
    return false;
}
// --- Damage tracking and kill/assist awarding ---
function recordDamage(victimId, attackerId, amount, now) {
    if (!victimId || !attackerId)
        return;
    let m = damageByVictim.get(victimId);
    if (!m) {
        m = new Map();
        damageByVictim.set(victimId, m);
    }
...
function handleDeathAndAwards(victim, killerId, now) {
    const victimScore = Math.max(0, victim.score | 0);
    const victimId = victim.id;
    const contrib = damageByVictim.get(victimId) || new Map();
    // Consider only those who damaged within the assist window
    const recent = [];
    for (const [aid, rec] of contrib.entries()) {
        if (now - rec.last <= ASSIST_WINDOW_MS)
            recent.push({ id: aid, dmg: rec.dmg, last: rec.last });
    }
    // Ensure killer is present in the set
    if (!recent.find(r => r.id === killerId))
        recent.push({ id: killerId, dmg: 0, last: now });
    ...
    if (mode === 'solo') {
        ...
        if (killerId === majority.id) {
            killerGain = majGain;
            assistGain = minGain;
            assisterId = minority.id;
        }
        else if (killerId === minority.id) {
            killerGain = minGain;
            assistGain = majGain;
            assisterId = majority.id;
        }
        else {
    ...
    if (killer)
        payload.killer = { id: killer.id, username: killer.username, gain: killerGain };
    if (assisterId) {
        const assister = gameState.players[assisterId];
        if (assister)
            payload.assister = { id: assister.id, username: assister.username, gain: assistGain };
    }
    io.emit('feed:kill', payload);
    // Local top notifications (5s)
    const killerSock = io.sockets.sockets.get(killerId);
    if (killerSock)
        killerSock.emit('notify', { type: 'kill', text: `You killed ${victim.username}`, ttlMs: 5000 });
    if (assisterId) {
        const assister = gameState.players[assisterId];
...
function updateFoods(dt) {
    ...
    for (const f of Object.values(gameState.foods)) {
        ...
        if (f.x > MAP_SIZE - FOOD_HALF) {
            f.x = MAP_SIZE - FOOD_HALF;
            if (typeof f.vx === 'number')
                f.vx = -Math.abs(f.vx);
        }
        if (f.y > MAP_SIZE - FOOD_HALF) {
            f.y = MAP_SIZE - FOOD_HALF;
            if (typeof f.vy === 'number')
                f.vy = -Math.abs(f.vy);
        }
...
io.on('connection', (socket) => {
    ...
    socket.on('player:join', (username) => {
        ...
        if (LEVEL_STEPS.length > 0)
            socket.emit('levels:init', LEVEL_STEPS);
        ...
        if (me) {
            ...
            // unguarded: rely on per-player masks (per-player masks checked inside sharkCollidesWithShark)
            {
                ...
                for (const other of Object.values(gameState.players)) {
                    if (other.id === socket.id || other.dead)
                        continue;
                ...
                // If movement is blocked, apply a two-sided knockback (no cooldown) and ensure separation
                if (blocked && hitOther) {
                    ...
                    if (meSock)
                        meSock.emit('shark:collision', { damage: SHARK_COLLISION_DAMAGE, x: nx, y: ny });
                    if (otherSock)
                        otherSock.emit('shark:collision', { damage: SHARK_COLLISION_DAMAGE, x: ox0, y: oy0 });
                    ...
                    if (me.hp <= 0 && !me.dead) {
                        ...
                        s?.emit('player:died');
                        try {
                            handleDeathAndAwards(me, hitOther.id, now);
                        }
                        catch { }
                        // Remove bubbles and state
                        for (const bid of Object.keys(bubbles)) {
                            if (bubbles[Number(bid)]?.ownerId === me.id) {
                                delete bubbles[Number(bid)];
                                projDirty = true;
                            }
                        }
                    ...
                    if (hitOther.hp <= 0 && !hitOther.dead) {
                        ...
                        s?.emit('player:died');
                        try {
                            handleDeathAndAwards(hitOther, me.id, now);
                        }
                        catch { }
                        // Remove bubbles and state
                        for (const bid of Object.keys(bubbles)) {
                            if (bubbles[Number(bid)]?.ownerId === hitOther.id) {
                                delete bubbles[Number(bid)];
                                projDirty = true;
                            }
                        }
    ...
    socket.on('disconnect', () => {
        ...
        for (const id of Object.keys(bubbles)) {
            if (bubbles[Number(id)]?.ownerId === socket.id)
                delete bubbles[Number(id)];
        }
            ...
            for (const p of Object.values(gameState.players)) {
                ...
                if (bubbleHitsShark(p, checkX, checkY)) {
                    ...
                    if (p.hp <= 0) {
                        ...
                        // Award kill/assists
                        try {
                            handleDeathAndAwards(p, b.ownerId, now);
                        }
                        catch { }
                        // Remove this player's active bubbles and cooldown/state
                        for (const bid of Object.keys(bubbles)) {
                            if (bubbles[Number(bid)]?.ownerId === dyingId) {
                                delete bubbles[Number(bid)];
                                projDirty = true;
                            }
                        }
...
async function preloadAllSharkMasks() {
    if (!SHARK_EVOLUTIONS || SHARK_EVOLUTIONS.length === 0) {
...
const PORT = envPort ? parseInt(envPort, 10) : 3002;
