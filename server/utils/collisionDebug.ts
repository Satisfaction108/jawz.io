...
export function visualizeMask(mask: Uint8Array, size: number, label: string = 'Mask'): void {
  console.log(`\n=== ${label} (${size}x${size}) ===`);
  
  // Count opaque pixels
  let opaqueCount = 0;
  for (let i = 0; i < size * size; i++) {
    if (mask[i] !== 0) opaqueCount++;
  }
...
export function testCollisionPoint(
  mask: Uint8Array,
  maskSize: number,
  worldX: number,
  worldY: number,
